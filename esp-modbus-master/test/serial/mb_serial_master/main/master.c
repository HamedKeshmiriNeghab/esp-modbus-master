...
#define MB_PORT_NUM     (/*CONFIG_MB_UART_PORT_NUM*/2)   // Number of UART port used for Modbus connection
#define MB_DEV_SPEED    (/*CONFIG_MB_UART_BAUD_RATE*/115200)  // The communication speed of the UART
    ...
    for(uint16_t retry = 0; retry <= MASTER_MAX_RETRY && (!alarm_state); retry++) {
        ...
        for (uint16_t cid = 0; (err != ESP_ERR_NOT_FOUND) && cid < MASTER_MAX_CIDS; cid++)
        {
            ...
            if ((err != ESP_ERR_NOT_FOUND) && (param_descriptor != NULL)) {
                ...
                vTaskDelay(/*POLL_TIMEOUT_TICS*/10); // timeout between polls
            }
        }
...
static esp_err_t master_init(void)
{
    ...
    err = uart_set_pin(MB_PORT_NUM, /*CONFIG_MB_UART_TXD*/8, /*CONFIG_MB_UART_RXD*/9,
                              /*CONFIG_MB_UART_RTS*/10, UART_PIN_NO_CHANGE);
